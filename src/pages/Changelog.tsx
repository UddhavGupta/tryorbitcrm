...
const RELEASES: Release[] = [
  ...
  {
    version: "v1.8",
    title: "Sheets import, Granola sync, brief abuse monitoring",
    date: "Jun 2026",
    status: "shipped",
    items: [
      "Google Sheets import: paste a published-to-web CSV link, preview rows, and populate People + Groups in one click (groups are auto-created as needed)",
      "Downloadable CSV template so you can start from a clean structure",
      "Granola integration: 'Sync now' button pulls recent meeting notes via the connector, matches attendees to existing contacts by email, and logs each meeting as an interaction",
      "Unmatched Granola attendees land in a per-user review queue — one click to add them as a new contact or dismiss",
      "Demo transcript button so the Granola flow is fully explorable without connecting Granola",
      "Shared brief URLs now route through an edge function that logs every access (hashed IP, user agent, country) to a service-role-only table for scraping detection",
      "Public brief responses send X-Robots-Tag: noindex to keep share links out of search engines",
    ],
  },
  {
    version: "v1.9",
  ...
  },
  {
    ...
    items: [
      ...
      "Per-token rate limiting on /brief/:token once Postgres-backed primitives land",
